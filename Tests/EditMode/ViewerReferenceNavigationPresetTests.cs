...
{
    public sealed class ViewerReferenceNavigationPresetTests
    {
        [Test]
        public void TemplateResolvesTheSharedReferenceProfiles()
        {
            ...
            try
            {
                ReferenceViewerBootstrap bootstrap =
                    root.AddComponent<ReferenceViewerBootstrap>();
        ...
        public void TemplateDelegatesScreenSpaceLayeringToUiPackage()
        {
            UnityEditor.PackageManager.PackageInfo package =
                UnityEditor.PackageManager.PackageInfo.FindForAssembly(
                    typeof(ReferenceViewerBootstrap).Assembly);
        ...
        private static void InstallReferenceStack(
            ...
            out ViewerShellPresenter shell)
        {
            MethodInfo installRendering = typeof(ViewerBootstrap).GetMethod(
                "ComposeRendering",
                BindingFlags.Instance | BindingFlags.NonPublic);
            MethodInfo installNavigation = typeof(ViewerBootstrap).GetMethod(
                "ComposeReferenceNavigation",
                BindingFlags.Instance | BindingFlags.NonPublic);
            MethodInfo installShell = typeof(ViewerBootstrap).GetMethod(
                "ComposeShell",
                BindingFlags.Instance | BindingFlags.NonPublic);
            ...
            rendering = (ViewerRenderingInstaller)installRendering.Invoke(
                bootstrap,
                null);
            object referenceNavigation = installNavigation.Invoke(
                bootstrap,
                new object[] { rendering });
            Assert.That(referenceNavigation, Is.TypeOf<
                ViewerNavigationReferenceAdapter>());
            navigation = bootstrap.NavigationInstaller;
