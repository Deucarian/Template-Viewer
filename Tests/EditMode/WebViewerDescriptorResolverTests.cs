...
{
    public sealed class WebViewerDescriptorResolverTests
    {
        [Test]
        public void PreservesExplicitVersionAndCacheMetadata()
        {
            var resolver = new DirectWebViewerModelDescriptorResolver();
            var request = new WebViewerInitializeRequest
            {
                Revision = 4,
            ...
            Assert.That(
                resolver.TryResolve(request, out WebViewerModelDescriptor value, out string error),
                Is.True,
                error);
        ...
        public void RejectsUnsafeAbsoluteSchemes(string source)
        {
            ...
            Assert.That(
                resolver.TryResolve(
                    new WebViewerInitializeRequest { Revision = 1, ModelUrl = source },
                    out _,
                    out _),
                Is.False);
        }
