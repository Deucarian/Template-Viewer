...
{
    [Serializable]
    public sealed class ViewerCommandHarnessScenario
    {
        public ViewerCommandHarnessScenario(
            string id,
            string label,
            string commandName,
    ...
    [Serializable]
    public sealed class ViewerCommandHarnessCatalog
    {
        internal ViewerCommandHarnessCatalog(
            IReadOnlyList<ViewerCommandHarnessScenario> scenarios,
        ...
        public IReadOnlyList<ViewerCommandHarnessScenario> Scenarios
        {
            get;
        }
    }

    public static class ViewerCommandHarnessCatalogBuilder
    {
        public static ViewerCommandHarnessCatalog Create(
            IEnumerable<ICommandHandler<ViewerApplication>> handlers,
            IEnumerable<ViewerCommandHarnessScenario> scenarios)
        {
            if (handlers == null)
            {
            ...
            foreach (ICommandHandler<ViewerApplication> handler in handlers)
            {
            ...
            var result = new List<ViewerCommandHarnessScenario>();
            string defaultScenarioId = string.Empty;
            if (scenarios != null)
            {
                foreach (ViewerCommandHarnessScenario scenario in scenarios)
                {
                    if (scenario == null)
                    {
                        throw new ArgumentException(
            ...
            foreach (string commandName in commandNames)
            {
                ...
                result.Add(new ViewerCommandHarnessScenario(
                    id,
            ...
            return new ViewerCommandHarnessCatalog(
                result,
                defaultScenarioId);
        }

        public static IReadOnlyList<ViewerCommandHarnessScenario>
        ...
        private static ViewerCommandHarnessScenario Scenario(
            string id,
            string label,
            string commandName,
            JObject payload = null,
